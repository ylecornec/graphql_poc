...
let get_string = function
  | `String s -> s
  | json -> failwith @@ Format.asprintf "expecting a string as json argument (%s)\n%s\n" __LOC__ (Yojson.Basic.to_string json)

let get_int = function
  | `Int i -> i
  | json -> failwith @@ Format.asprintf "expecting an int as json argument (%s)\n%s\n" __LOC__ (Yojson.Basic.to_string json)
